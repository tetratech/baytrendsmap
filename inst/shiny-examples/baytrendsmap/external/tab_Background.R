...
function() {
  tabPanel("Background"
    ...
    , p("The GAM approach enables us to describe non-linear seasonally varying changes over time, to incorporate hydrologic variability via either river flow or salinity, to accommodate method or laboratory changes suspected to impact data values, and to represent left- and interval-censored data (Murphy et al, 2019, 2021). The baytrends R package was designed to fit GAMs for the tidal Chesapeake Bay water quality data. The package has already been used by MDNNR, VADEQ (via analysts at Old Dominion University, ODU), Department of Energy and Environment (DOEE), and Metropolitan Washington Council of Governments (COG) and can be downloaded through the CRAN for anyone interested in the analyses.")
    , p(tags$em("Long- and short-term changes"))
    ...
    , p(tags$em("Flow/salinity-adjustment procedure"))
    ...
    , p(tags$em("Laboratory and method changes"))
